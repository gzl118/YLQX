...
function dialogAjaxDone(json) {
    ...
    if (json.statusCode == DWZ.statusCode.ok) {
        ...
        if (json.callbackType == "closeCurrentReloadTab") {
            $.pdialog.closeCurrent();
            navTab.reload(json.forwardUrl);
        }
        // add by gaokang 2015/6/5
        if (json.callbackType == "closeDialogReloadTabOrDialog") {
            $.pdialog.closeCurrent();
            var isDialog = $("body").data(json.dlgid);
            if (isDialog) {
                $.pdialog.open(json.forwardUrl, json.dlgid);
            } else {
                navTab.reload(json.forwardUrl);
            }
        }
...
// add by gaokang 2015/6/5
function MySearch(form, dlgid) {
    var $form = $(form);
    if (form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
    var isDialog = $("body").data(dlgid);
    if (isDialog) {
        $.pdialog.reload($form.attr('action'), { data: $form.serializeArray() });
    } else {
        navTab.reload($form.attr('action'), { data: $form.serializeArray(), navTabId: undefined });
    }
    return false;
