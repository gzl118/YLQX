...
namespace MedicalApparatusManage.Controllers
{
    public class T_CKDController : Controller
    {
        ...
        public ActionResult Details(System.Int32 id)
        {
            T_CKDModels model = new T_CKDModels();
            model.DataModel = T_CKDDomain.GetInstance().GetModelById(id);
            if (model.DataModel.XSID != 0)
            {
                var xsd = T_XSDDomain.GetInstance().GetModelById(model.DataModel.XSID);
                model.XSDMC = xsd == null ? "" : xsd.XSMC;
            }
            model.CKMXList = T_CKMXDomain.GetInstance().GetT_CKMXByCkid(id);
        ...
        public void ExportExcel(System.Int32 id)
        {
            ...
            qy = T_WhsQYDomain.GetInstance().GetModelById(3);
            ...
            //打印列名
            sHtml.Append("<tr height=\"30\" align=\"center\" ><td>产品名称</td><td>产品规格（型号）</td><td>生产企业</td><td>单位</td><td>数量</td><td>单价</td>"
            ...
            for (int i = 0; i < ckmxList.Count; i++)
            {
                ...
                if (ckmx.CPYXQ != null)
                {
                    scRq = ckmx.CPYXQ.Value.ToLongDateString();
                }
                //生产企业
            ...
            sHtml.Append("</table>");
            sHtml.Append("<table  border=\"0\" width=\"100%\">");
            sHtml.Append("<tr height=\"40\" align=\"center\"><td colspan=\"7\" align=\"left\">销售员：&nbsp;&nbsp</td><td align=\"right\">复核员：&nbsp;&nbsp</td><td align=\"right\">出库员：&nbsp;&nbsp</td><td align=\"center\">收货员：&nbsp;&nbsp</td></tr>");
            sHtml.Append("</table>");
            //调用输出Excel表的方法
            ExportToExcel("application/vnd.ms-excel", qy.WhsMC + "销售复核出库单.xls", sHtml.ToString());
        }
